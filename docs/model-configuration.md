...
| Oracle | `fable-5` | 原生 `claude` 订阅 | 架构裁决、疑难根因和高风险 diff 的只读复核 |
...
网关精确模式需要 `/v1/models` 返回 `config/models.json` 派生的七个 ID：`gpt-5.6-sol`、`gpt-5.6-luna`、`gpt-5.6-terra`、`grok-4.5`、`kimi-k3`、`gemini-3.5-flash`、`glm-5.2`。`fable-5` 走原生订阅，不参加网关目录检查。requested 与 resolved identity 不一致应视为失败，不能静默 fallback。
